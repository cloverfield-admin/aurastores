-- Align advisory lock namespace with AuraStores (deploy with app that uses the same prefix).
...
BEGIN
  ...
  PERFORM pg_advisory_xact_lock(hashtext('aurastores:sales_monthly:' || NEW.organization_id::text));
