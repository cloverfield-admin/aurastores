      ...
      <div className="w-full max-w-lg rounded-2xl bg-[var(--app-surface)] p-6 shadow-2xl">
        <div className="mb-5">
          ...
          <p className="mt-1 text-sm text-[var(--app-text-muted)]">
            Apply one adjustment to {batchCount} product
            {batchCount === 1 ? "" : "s"}.
          </p>
        </div>
        ...
        <div className="space-y-4">
          {batchCount > 1 ? (
            <div className="rounded-xl bg-[var(--app-surface-muted)] p-3">
              ...
              <p className="mt-2 text-[11px] text-[#b45309]">
                This adjustment applies the same quantity change to all selected
                products.
              </p>
            </div>
          ) : null}
          ...
          {successMessage ? (
            <p className="text-sm font-medium text-[var(--app-link-teal)]">
              {successMessage}
            </p>
          ) : null}
        </div>
          ...
          <button
            ...
            onClick={async () => {
              ...
              if (!Number.isFinite(quantityDelta) || quantityDelta === 0) {
                setError(
                  "Enter a non-zero whole number for the stock adjustment.",
                );
                return;
              }
              ...
              try {
                ...
                if (isOfflineQueuedError(submitError)) {
                  setSuccessMessage(
                    "Adjustment queued — it will sync when you are back online.",
                  );
                  window.setTimeout(() => onClose(), 900);
                  return;
                }
      ...
      <div className="flex gap-3">
        ...
        <div className="min-w-0 flex-1 space-y-3">
          ...
          <dl className="grid grid-cols-[auto_1fr] gap-x-3 gap-y-1.5 text-xs">
            {showBranchColumn ? (
              <>
                <dt className="font-semibold text-[var(--app-text-faint)]">
                  Branch
                </dt>
...
export function StockInventoryContent() {
  ...
  const [optimisticAdjustments, setOptimisticAdjustments] = useState<
    Record<
      string,
      {
        ...
        status: StockDashboardResponse["inventory"][number]["status"];
      }
    >
  >({});
  const [recentlyAdjustedBatchIds, setRecentlyAdjustedBatchIds] = useState<
    string[]
  >([]);
  ...
  const [inventoryStatus, setInventoryStatus] = useState<
    "all" | "out_of_stock" | "reorder_attention"
  >(() => {
            ...
            row.quantityReceived > 0
              ? Math.max(
                  ...
                  Math.min(
                    100,
                    Math.round(
                      (optimistic.quantityAvailable / row.quantityReceived) *
                        100,
                    ),
                  ),
                )
  ...
  const allRowsSelected =
    ...
    selectableRowIds.every((rowId) => selectedBatchIds.includes(rowId));

  useEffect(() => {
    queueMicrotask(() => {
      setSelectedBatchIds((current) =>
        current.filter((batchId) => rowById.has(batchId)),
      );
    });
  }, [rowById]);
  ...
  async function runAdjustment(
    ...
    return withLoading(
      ...
      async () => {
        ...
        setSelectedBatchIds((current) =>
          current.filter((batchId) => !batchIds.includes(batchId)),
        );
        notify({
          variant: "success",
        ...
        setRecentlyAdjustedBatchIds(
          result.updatedBatches.map((batch) => batch.id),
        );
    ...
    <div className="relative px-4 pb-24 sm:px-6 lg:px-8">
      <div className="mx-auto max-w-[1280px] space-y-10">
              ...
              onClick={async () => {
                ...
                if (validBatchIds.length === 0) {
                  notify({
                    ...
                    description:
                      "Select at least one active or expiring product to run bulk adjustment.",
                  });
                  return;
                }
            ...
            const cardContent = (
              <>
                <div className="mb-4 flex items-start justify-between">
                  ...
                  <span
                    className={`rounded-full px-2 py-1 text-xs font-semibold ${metric.badgeClass}`}
                  >
                    {metric.badge}
                  </span>
                </div>
            ...
            if (isOutOfStock) {
              ...
              return (
                <button
                  ...
                  title={
                    isActive
                      ? "Clear reorder-attention filter"
                      : "Show products needing reorder attention"
                  }
                >
                  {cardContent}
                </button>
        ...
        <section className="overflow-hidden rounded-xl border border-[var(--app-border)] bg-[var(--app-surface)] shadow-sm">
          <div className="flex flex-col gap-4 border-b border-[var(--app-input-bg)] p-6">
            ...
            <div className="flex flex-wrap items-center justify-between gap-3 text-[11px] uppercase tracking-[0.1em] text-[var(--app-text-faint)]">
              ...
              <span>
                Branch: {stockQuery.data?.branch.name ?? "Loading..."}
              </span>
            </div>
          </div>
                ...
                <div className="md:hidden border-t border-[var(--app-surface-subtle)] bg-[var(--app-surface)] px-4 pb-4 pt-3">
                      ...
                      <input
                        ...
                        onChange={() => {
                          setSelectedBatchIds((current) =>
                            allRowsSelected
                              ? current.filter(
                                  (id) => !selectableRowIds.includes(id),
                                )
                              : Array.from(
                  ...
                  <div className="space-y-3">
                    {rows.map((row) => {
                      ...
                      const isRecentlyAdjusted =
                        recentlyAdjustedBatchIds.includes(row.id);
                      return (
                        <StockInventoryMobileCard
                          ...
                          onEdit={() => {
                            ...
                            const href = branchParam
                              ...
                              : ROUTES.dashboard.stockProductEdit(
                                  row.productId,
                                );
                            router.push(href);
                          }}
                          ...
                          onDispose={() => {
                            void withLoading(
                              ...
                              async () => {
                                const result =
                                  await disposeBatchMutation.mutateAsync({
                                    batchId: row.id,
                                    branchId,
                                    note:
                                      row.status === "expired"
                          ...
                          onRestore={() => {
                            void withLoading(
                              ...
                              async () => {
                                const result =
                                  await restoreBatchMutation.mutateAsync({
                                    batchId: row.id,
                                    branchId,
          ...
          <div className="flex flex-col items-center justify-between gap-4 border-t border-[var(--app-surface-subtle)] bg-[rgba(242,244,246,0.3)] px-6 py-4 sm:flex-row">
            <p className="text-[10px] font-semibold uppercase tracking-wider text-[var(--app-text-faint)]">
              Showing page {pagination.page.toLocaleString()} of{" "}
            ...
            <div className="flex items-center gap-3">
              ...
              <button
                type="button"
                disabled={
                  pagination.page >= pagination.totalPages || isPageLoading
                }
                onClick={() => {
                  setPage((current) =>
        ...
        <footer className="flex flex-col gap-4 border-t border-[var(--app-surface-subtle)] pt-6 text-[11px] uppercase tracking-[0.1em] text-[var(--app-text-faint)] sm:flex-row sm:items-center sm:justify-between">
          ...
          <div className="flex flex-wrap gap-4">
            <Link
              href="#"
              className="underline decoration-[rgba(20,184,166,0.3)] hover:text-[var(--app-text-muted)]"
            >
              Privacy Policy
            </Link>
            <Link
      ...
      <StockAdjustDialog
        ...
        selectedBatchLabels={adjustDialog.batchIds.map((batchId) => {
          const row = rowById.get(batchId);
          return row
            ? `${row.productName} (#${row.batchNumber})`
            : `Product ${batchId}`;
        })}
