...
import { useCreateOrganizationBranchMutation } from "@/lib/queries/organization";
...
export function NewBranchForm() {
  ...
  const createBranchMutation = useCreateOrganizationBranchMutation();

  const [hoursMode, setHoursMode] = useState<HoursMode>("custom");
  const [branchName, setBranchName] = useState("");
  const [pharmacistCount, setPharmacistCount] = useState("1");
  ...
  const isBusy = isLoading("dashboard-new-branch");
  ...
  async function handleSubmit(e: FormEvent<HTMLFormElement>) {
    ...
    try {
      const parsedCount = Number.parseInt(pharmacistCount, 10);
      ...
      notify({
        ...
        description: "Your new branch has been added to the organization.",
      });
      router.push(ROUTES.dashboard.organization);
    } catch (submitError) {
      const message = submitError instanceof Error ? submitError.message : "Could not create branch.";
      setError(message);
      notify({
        variant: "error",
      ...
      <div className="mx-auto max-w-[1280px]">
        ...
        >
          <fieldset disabled={isBusy} className="contents">
            ...
            <div className="flex flex-col gap-6 xl:flex-row xl:items-stretch">
              ...
              <div
                className={`flex flex-1 flex-col rounded-[20px] bg-[#f2f4f6] p-6 xl:min-w-0 ${
                  mapExpanded
                    ? "min-h-[min(88dvh,900px)] xl:min-h-[min(82dvh,900px)]"
                    : "min-h-[320px] xl:min-h-0"
                }`}
              >
                ...
                <p className="mt-2 text-xs leading-relaxed text-[var(--app-text-muted)]">
                  Use search or click to set a pin. The formatted address resolves into the branch location field for saving.
                </p>
            ...
            <div className="flex flex-col gap-8 rounded-[20px] bg-[#f2f4f6] p-8">
              <div className="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
                <div className="space-y-1">
                  ...
                  <p className="text-sm text-[#3c4948]">When will this branch be active?</p>
                </div>
            ...
            {error ? (
              <AuraInlineAlert variant="error" title="Could not create branch" description={error} />
            ) : null}
            ...
            <div className="flex flex-col-reverse items-stretch justify-between gap-4 border-t border-[rgba(187,201,199,0.2)] pt-10 sm:flex-row sm:items-center">
              <Link
                ...
                className={`inline-flex items-center justify-center gap-2 rounded-2xl bg-[#f2f4f6] px-8 py-3 text-base font-semibold text-[#3c4948] transition hover:bg-[#e8eaed] ${
