...
import { payPaymentMethodSchema } from "@/lib/validation/pay";
...
export async function GET(request: Request) {
  ...
  const parsedMethod = methodParam ? payPaymentMethodSchema.safeParse(methodParam) : null;
  if (parsedMethod && !parsedMethod.success) {
    return NextResponse.json({ error: "Invalid payment method filter." }, { status: 400 });
  }

  try {
    const dashboard = await services.pay.getDashboard(gate.context, {
      branchId,
    ...
    return NextResponse.json(
      { error: error instanceof Error ? error.message : "Unable to load Aura Pay." },
      { status: 400 },
    );
  }
}
